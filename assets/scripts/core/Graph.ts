import Events, { NodesEvent, GraphEvent, NodeConnectEvent, NodeConnectEventArg, NodeEvent, GroupEvent } from './misc/Events';

import NodeBase, { NodeJSONType } from './nodes/NodeBase';
import { Nodes } from './nodes/index';
import Group, { GroupJSONType } from './Group';
...
export default class Graph implements IDisposable {
  ...
  constructor (nodes: NodeBase[] = [], groups: Group[] = []) {
  ...
  public createGroupsJSON (json: GroupJSONType[]) : Group[] {
    return json.map((el) => {
      const group = new Group(el.uuid, el.nodes);
      this.addGroup(group);
      return group;
    });
  }
