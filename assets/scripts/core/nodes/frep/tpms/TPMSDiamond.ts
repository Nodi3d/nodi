...
export default class TPMSDiamond extends FrepNodeBase {
  public get displayName (): string {
    return 'Diamond';
  }

  public registerInputs (manager: InputManager): void {
    manager.add('f', 'Frep to schwarz D', DataTypes.FREP, AccessTypes.ITEM);
  ...
  public solve (access: DataAccess): void {
    ...
    const code = function (p: string): string {
      ...
      const g2 = `opDiamond(${p}, ${scale.toFixed(4)}, ${thickness.toFixed(4)}, ${bias.toFixed(4)})`;
