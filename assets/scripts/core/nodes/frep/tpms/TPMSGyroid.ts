...
export default class TPMSGyroid extends FrepNodeBase {
  public get displayName (): string {
    return 'Gyroid';
  }

  public registerInputs (manager: InputManager): void {
    manager.add('f', 'Frep to gyroid', DataTypes.FREP, AccessTypes.ITEM);
  ...
  public solve (access: DataAccess): void {
    ...
    const code = function (p: string): string {
      ...
      const g2 = `opGyroid(${p}, ${scale.toFixed(4)}, ${thickness.toFixed(4)}, ${bias.toFixed(4)})`;
