...
export default class FrenetFrame {
  origin: NPoint;
  tangent: Vector3;
  normal: Vector3;
  binormal: Vector3;
...
const computeFrenetFrames = function (points: NPoint[], tangents: Vector3[], closed = false): FrenetFrame[] {
  ...
  for (i = 1; i <= segments; i++) {
    ...
    if (vec.length() > Number.EPSILON) {
      vec.normalize();
      theta = Math.acos(Helper.clamp(tangents[i - 1].dot(tangents[i]), -1, 1)); // clamp for floating pt errors
  ...
  if (closed) {
    theta = Math.acos(Helper.clamp(normals[0].dot(normals[segments]), -1, 1));
    theta /= segments;
