...
import { GroupViewEvent, GroupViewEventArg, GUIEvent, IOViewEvent, NodeIOViewEvent, NodeIOViewEventArg, NodeViewEvent, NodeViewEventArg } from './misc/Events';
...
export default class Editor implements IDisposable {
  ...
  public addGroup (uuid: string, nodes: string[]): Group {
    const group = new Group(uuid, nodes);
  ...
  public addToGroup (group: Group, nodes: string[]): void {
    const op = new Operations.AddToGroupOperation(group.uuid, nodes);
    this.pushHistory(op).do(this);
  }

  public removeFromGroup (group: Group, nodes: string[]): void {
