...
export class TPMSFischerKochS extends FrepNodeBase {
  public get displayName (): string {
    return 'Fischer Koch S';
  }

  public registerInputs (manager: InputManager): void {
    manager.add('f', 'Frep to fischer koch', DataTypes.FREP, AccessTypes.ITEM);
  ...
  public solve (access: DataAccess): void {
    ...
    const code = function (p: string): string {
      ...
      const g2 = `opFischerKoch(${p}, ${scale.toFixed(4)}, ${thickness.toFixed(4)}, ${bias.toFixed(4)})`;
